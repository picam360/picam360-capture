...
#define PLUGIN_NAME "gst_decoder"
...
static void *pout_thread_func(void* arg) {
	...
	gst_decoder *_this = (gst_decoder*) arg;
...
static void create_decoder(void *user_data, DECODER_T **out_decoder) {
	DECODER_T *decoder = (DECODER_T*) malloc(sizeof(gst_decoder));
	memset(decoder, 0, sizeof(gst_decoder));
