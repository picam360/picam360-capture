...
OMXJPEG_FN_DEFINE(void, jpeg_CreateDecompress,
	...
	omx_jpeg_decompress_private *_this = (omx_jpeg_decompress_private*) malloc(sizeof(omx_jpeg_decompress_private));
	...
	// callback
	ilclient_set_fill_buffer_done_callback(_this->client, my_fill_buffer_done, (void*) _this);

	// create video_decode
	if (ilclient_create_component(_this->client, &_this->video_decode, (char*) "video_decode", (ILCLIENT_CREATE_FLAGS_T)(ILCLIENT_DISABLE_ALL_PORTS | ILCLIENT_ENABLE_INPUT_BUFFERS)) != 0)
		status = -14;
	_this->list[0] = _this->video_decode;

	// create resize
	if (status == 0 && ilclient_create_component(_this->client, &_this->resize, (char*) "resize", (ILCLIENT_CREATE_FLAGS_T)(ILCLIENT_DISABLE_ALL_PORTS)) != 0)
		status = -14;
	_this->list[1] = _this->resize;

	// create lg_egl_render
	if (status == 0
			&& ilclient_create_component(_this->client, &_this->egl_render, (char*) "egl_render", (ILCLIENT_CREATE_FLAGS_T)(ILCLIENT_DISABLE_ALL_PORTS | ILCLIENT_ENABLE_OUTPUT_BUFFERS)) != 0)
		status = -14;
	...
	if (status == 0
			&& OMX_SetParameter(ILC_GET_HANDLE(_this->video_decode), OMX_IndexParamVideoPortFormat, &format) != OMX_ErrorNone){
		status = -14;
	}
	if (status == 0
			&& ilclient_enable_port_buffers(_this->video_decode, 130, NULL, NULL, NULL) != 0) {
		status = -14;
	}
...
OMXJPEG_FN_DEFINE(void, jpeg_destroy_decompress, (j_decompress_ptr cinfo)) {
	omx_jpeg_decompress_private *_this = (omx_jpeg_decompress_private*) cinfo->master;
	...
	if (OMX_EmptyThisBuffer(ILC_GET_HANDLE(_this->video_decode), buf) != OMX_ErrorNone)
		status = -20;
...
OMXJPEG_FN_DEFINE(int, jpeg_read_header,
	...
	OMX_GetParameter(ILC_GET_HANDLE(_this->resize), OMX_IndexParamPortDefinition, &portdef);

	portdef.format.image.nFrameWidth = image_width;
	portdef.format.image.nFrameHeight = image_height;

	OMX_SetParameter(ILC_GET_HANDLE(_this->resize), OMX_IndexParamPortDefinition, &portdef);
	...
	OMX_SetConfig(ILC_GET_HANDLE(_this->resize), OMX_IndexConfigCommonInputCrop, &omx_crop_req);
	...
	omx_err = OMX_GetParameter(ILC_GET_HANDLE(_this->egl_render), OMX_IndexParamPortDefinition, &port_format);
	...
	omx_err = OMX_SetParameter(ILC_GET_HANDLE(_this->egl_render), OMX_IndexParamPortDefinition, &port_format);
	if (omx_err != OMX_ErrorNone) {
	...
	if (OMX_SendCommand(ILC_GET_HANDLE(_this->egl_render), OMX_CommandPortEnable, 221, NULL) != OMX_ErrorNone) {
...

	{
		...
		if (state != OMX_StateIdle) {
			if (state != OMX_StateLoaded) {
				ilclient_change_component_state(_this->egl_render, OMX_StateLoaded);
			}
			ilclient_change_component_state(_this->egl_render, OMX_StateIdle);
		}
		omx_err = OMX_UseEGLImage(ILC_GET_HANDLE(_this->egl_render), &_this->egl_buffer, 221, (void*) 0, _this->egl_image);
		if (omx_err != OMX_ErrorNone) {
...
OMXJPEG_FN_DEFINE(JDIMENSION, jpeg_read_raw_data,
	...
	if (OMX_FillThisBuffer(ilclient_get_handle(_this->egl_render), _this->egl_buffer) != OMX_ErrorNone) {
