...
static void dump_portdef(OMX_PARAM_PORTDEFINITIONTYPE* portdef) {
	...
	case OMX_PortDomainVideo:
		say("Video type:\n"
				...
				viddef->nStride, viddef->nSliceHeight, viddef->nBitrate,
	...
	if (dumpformats) {
		...
		while (r == OMX_ErrorNone) {
			if ((r = OMX_GetParameter(hComponent, OMX_IndexParamVideoPortFormat,
				...
				say("\t%s, compression: %s",
						dump_color_format(portformat.eColorFormat),
...
static void init_component_handle(const char *name, OMX_HANDLETYPE* hComponent,
		OMX_PTR pAppData, OMX_CALLBACKTYPE* callbacks) {
	OMX_ERRORTYPE r;
	char fullname[32];
		...
		if (OMX_GetParameter(*hComponent, types[i], &ports) == OMX_ErrorNone) {
			...
			for (nPortIndex = ports.nStartPortNumber;
				...
				if ((r = OMX_SendCommand(*hComponent, OMX_CommandPortDisable,
					...
					omx_die(r, "Failed to disable port %d of component %s",
							nPortIndex, fullname);
				}
...
void *video_direct(void* arg) {
	...
	say("Switching state of the camera component to executing...");
	if ((r = OMX_SendCommand(ctx.camera, OMX_CommandStateSet,
	...
	if ((r = OMX_SendCommand(ctx.render, OMX_CommandStateSet,
			OMX_StateExecuting, NULL)) != OMX_ErrorNone) {
		omx_die(r,
				"Failed to switch state of the render component to executing");
	}
	...
	if ((r = OMX_SendCommand(ctx.null_sink, OMX_CommandStateSet,
		...
		omx_die(r,
				"Failed to switch state of the null sink component to loaded");
	}
